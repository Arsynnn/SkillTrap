...
```bash
uv sync
uv run skilltrap scan fixtures/malicious/reads_env_canary   # сквозной прогон, вердикт MALICIOUS
uv run skilltrap eval                                       # все фикстуры + precision/recall
uv run pytest                                               # 77 тестов
uv run pytest -m "not docker"                               # совсем без Docker
uv run ruff check . && uv run ruff format .
```

Первый запуск собирает образ песочницы (`skilltrap-sandbox:latest`) — это пара минут.
...
### Шаг 11 — документация и сам скил (`docs: add skill definition, README and architecture docs`)

- `skill/skilltrap/SKILL.md` — SkillTrap как скил: когда вызывать, как читать вердикт,
  какие ограничения обязательно озвучить пользователю. Проверен собственным сканером: SAFE.
- `docs/architecture.md` — схема конвейера, таблица модулей и объяснение ключевых решений.
- `docs/threat_model.md` — что ловим, чего НЕ ловим (prompt injection, отложенное срабатывание,
  распознавание песочницы, другие языки), безопасность самой проверки, остаточный риск.
- `README.md` — проблема, схема, установка, таблица правил, результаты eval, ограничения, roadmap.
  В разделе «Проблема» ссылки на исследования помечены как «добавить перед публикацией»:
  придумывать URL нельзя, их нужно проверить вручную.

## Осталось

MVP по определению из CLAUDE.md закрыт. Дальше по убыванию пользы:

1. **Ссылки на исследования в README** — сейчас источники названы, но URL не проставлены:
   их нужно найти и проверить вручную, выдумывать ссылки нельзя.
2. **Демо** — GIF или скриншот отчёта в README (в README пока текстовый пример вывода).
3. **Больше чистых фикстур** — взять урезанные копии реальных скилов из `anthropics/skills`.
   Сейчас benign-фикстур всего две и обе написаны автором, поэтому precision 1.00 говорит
   лишь о том, что конвейер работает, а не о качестве на реальных скилах.
4. Этапы roadmap из CLAUDE.md: агент в контуре, GitHub Action, объединение со статическими
   сканерами, поддержка `.js`.

## Известные проблемы и обходные пути

- **`skilltrap eval` нужно запускать из корня репозитория.** Пакет `eval` не устанавливается
  (в wheel попадает только `skilltrap`), импорт работает через текущую папку.
- **Нужен Docker с Linux-контейнерами.** На Windows проверено на Docker Desktop; пути вида
  `C:\...` конвертируются в `C:/...` в `runner._mount()`.
- **strace обрезает длинные аргументы** на 256 символах (`-s 256`). Очень длинный секрет в
  аргументах `execve` может не попасть в лог целиком — правило `canary-value-leak` его пропустит.
  Обходной путь при необходимости: увеличить `-s` в `runner.TRACED_SYSCALLS`-команде.
- **Логи strace по умолчанию удаляются** вместе с временной папкой. Чтобы их сохранить:
  `uv run skilltrap scan <путь> --keep-workspace ./dump`.
